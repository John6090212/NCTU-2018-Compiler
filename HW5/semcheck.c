...
struct expr_sem *createExprSem(struct SymTable *table, const char *id ,int scope,__BOOLEAN currentScope) 
{
	...
	struct SymNode * node;
	node = lookupSymbol(table,id,scope,currentScope);
	result->node = node;
	result->isload = __FALSE;
	return result;
}
...
int insertParamIntoSymTable( struct SymTable *table, struct param_sem *params, int scope, int local_count)
{
			...
			if( parPtr->pType->isError == __TRUE ) { 
				...
				for( idPtr=(parPtr->idlist) ; idPtr!=0 ; idPtr=(idPtr->next) ) {
					if( verifyRedeclaration( table, idPtr->value, scope ) ==__FALSE ) { result = __TRUE;  }
						...
						newNode = createParamNode( idPtr->value, scope, parPtr->pType,local_count);
						local_count++;
