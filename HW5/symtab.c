...
struct SymNode* createVarNode( const char *name, int scope, struct PType *type, int local_number) 
{
	...
	/* without attribute */
	newNode->attribute = 0;
	newNode->local_number = local_number;
	newNode->next = 0;
	newNode->prev = 0;
...
struct SymNode* createParamNode( const char *name, int scope, struct PType *type, int local_number)
{
	struct SymNode *newNode = (struct SymNode *)malloc( sizeof(struct SymNode) );
	/* setup name */
	newNode->name = (char *)malloc(sizeof(char)*(strlen(name)+1));
	...
	for( i=0 ; i<HASHBUNCH ; i++ ) {
		for( ptr=(table->entry[i]) ; ptr!=0 ; ptr=(ptr->next) ) {
			if( ptr->scope == __scope ) {
				...
				printf("count = %d", ptr->local_number);
				printf("\n");
			}	// if( ptr->scope == __scope )
		}	// for( ptr=(table->entry[i]) ; ptr!=0 ; ptr=(ptr->next) )
	}	// for( i=0 ; i<HASHBUNCH ; i++ )
